...
class AnimateBBTree extends React.Component {
  ...
  getLine (ref) {
    return <svg ref={ref} viewBox='0 0 100 100' className='parentLine' preserveAspectRatio="none" style={{
      width: 0,
      height: 0,
      transform: 'scale(1,1)'
    }} >
      <path className='line' vectorEffect="non-scaling-stroke" stroke="black" strokeWidth="2" fill="transparent" d="M 2 0 Q 2 50, 26 50 T 74 50 T 98, 100"></path>
    </svg>
  ...
  animate (popNode=false) {
    ...
    const animationCallback = () => {
      ...
      nodes.forEach(([node, nodeData, level, offset, height]) => {
        ...
        let changed = false
        const [top, left] = [`${level*12 + 27}%`, `${100*(offset + 0.5)/(2**level)}%`]
        if (el.style.top !== top) {
          el.style.top = top
          changed = true
        }
      ...
      if ([
        ...
        const leftp = nodeData.ref.current.style['left']
  ...
  onAnimSpeedChange (e) {
    ...
    this.setState({animSpeed: e.target.value})
  ...
  render () {
    return (<div>
      ...
      <div className='tree' ref={this.treeref}>
        {this.domData.head}
        {this.domData.nodes}
        {this.domData.lines}
      </div>
